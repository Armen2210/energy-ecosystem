...
- в локальной версии `sitemap.xml` добавлены пять прямых страниц кейсов;
...
Публичные прямые маршруты кейсов:
...
Slug и имена папок кейсов намеренно обезличены. Не следует использовать в URL названия заказчиков, внутренние наименования объектов или коммерчески чувствительные географические привязки.
...
Локально проверены прямые маршруты кейсов:
...
После production-deploy эти маршруты необходимо повторно проверить на сервере, включая прямое открытие и обновление страницы.
...
- галереях объектов;
- общей структурой модального и прямого просмотра.
...
- данные для SEO и краткого машинопонятного описания;
- выполненные работы;
- особенности решения;
...
Текущая последовательность избранных кейсов на главной:

```text
БМК для спортивного объекта
...
Текущие опубликованные кейсы:

- БМК для спортивного объекта;
- БТП для предприятия пищевой промышленности;
- БТП для гостиничного комплекса;
- БТП для жилого комплекса;
- пароводяной БТП для производственного объекта.

Публичные URL, slug и папки изображений обезличены. Названия заказчиков и коммерчески чувствительные географические привязки не используются в маршрутах.
...
- центрирование просмотренного кейса на mobile;
- закрытие через крестик, фон и `Esc`;
- focus trap и возврат клавиатурного фокуса;
- галерея;
- переключение изображений шевронами и клавишами;
- mobile-свайп;
- предзагрузка соседних фотографий;
- плавная смена изображения без opacity- и blur-вспышек;
- адаптивные размеры;
- компактные метаданные;
- отдельные desktop- и mobile-CTA;
...
```text
src/components/CaseView/
```

Единое представление содержимого кейса используется одновременно:

- в `CaseModal`;
- на прямой странице `CasePage`.

Компонент содержит:

- галерею;
- счётчик и подпись фотографии;
- шевроны;
- mobile-свайп;
- клавиатурную навигацию;
- предзагрузку соседних изображений;
- метаданные объекта;
- задачу;
- перечень выполненных работ;
- desktop- и mobile-CTA.

Модальная оболочка, SEO, навигация страницы и управление закрытием вынесены из `CaseView` и остаются в соответствующих компонентах.
...
В `cases.js` сохранены данные `aiSummary` для будущего расширения машинопонятного представления кейсов, но текущий `CaseView` выводит компактную структуру без отдельного блока `AiSummary`.
...
В `sitemap.xml` добавлены все пять индексируемых прямых страниц кейсов с обезличенными slug.
...
- создан общий компонент `CaseView`;
- модальная и прямая версии кейса используют единое представление;
- заполнены пять полноценных кейсов;
- подключены галереи всех опубликованных кейсов;
- обезличены slug и папки изображений кейсов;
- добавлены все прямые страницы кейсов в `sitemap.xml`;
- настроена подборка из трёх избранных кейсов на главной;
- улучшена анимация смены фотографий без пустых, прозрачных и размытых кадров;
...
1. Выполнить production-deploy последних frontend-изменений.
2. Провести production smoke-test главной, `/cases`, пяти модальных и пяти прямых кейсов.
3. Проверить production-открытие новых обезличенных URL и SPA fallback.
4. Проверить обновлённый `sitemap.xml` на production и повторно отправить его в Яндекс Вебмастер и Google Search Console.
5. Оптимизировать изображения кейсов без потери инженерной информативности.
6. Повторить Lighthouse для главной, `/cases` и нескольких прямых кейсов.
7. Продолжить работу с Core Web Vitals, особенно LCP галерей.
8. Провести финальный редакторский аудит alt-текстов, подписей и публичной конфиденциальности кейсов.
9. При необходимости добавить структурированные данные для детальных страниц кейсов.
10. Постепенно разделить `App.css` по компонентам после завершения визуальной стабилизации.
11. Подключить аналитику после согласования cookies.
12. Передать юридический текст на финальную проверку юристу.
13. Расширять продуктовый и сервисный контент.
14. Подготовить будущие CRM-интеграции.
15. Отдельно рассмотреть page transition animation без риска для scroll-сценариев.
