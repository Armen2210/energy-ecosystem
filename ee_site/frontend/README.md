...
Последние frontend-изменения по mobile UX, услугам, кейсам, форме заявки, footer, EntitySwitcher, галереям и типографике проверены локально:

```text
npm run build — успешно
```

После следующего production-deploy необходимо повторно выполнить smoke-test основных маршрутов, прямых страниц кейсов, mobile-сценариев и формы заявки.
...
- единое отображение изображений галереи через заполнение выделенной рамки;
- исправление маршрута кейса БТП Дербент, чтобы карточка открывалась по `/cases/btp-residential-complex`.
...
На mobile переключатель работает как компактная sticky-навигация внутри продуктовых и сервисных страниц:
...
- продуктовый переключатель сохраняет формат `БМК / БТП / ВНС / ПНС / ШУиА`;
- переключатель услуг на mobile работает как segmented control `Проект / СМР / ПНР`;
- фон переключателя сделан легче и прозрачнее;
- системная голубая tap-подложка на touch-устройствах отключена через `global.css`.
...
Mobile-версия сервисных страниц приведена к логике продуктовых страниц:

- переключатель услуг работает как компактный segmented control;
- подпись `Проектирование` в переключателе сокращена до `Проект`;
- `СМР` и `ПНР` помещаются в одной строке с активным состоянием;
- для технического чертежа проектирования используется режим `document`;
- H1 на mobile уменьшен до более сбалансированного размера;
- блок заявки на страницах услуг уплотнён через секцию `#lead-form`.
...
Mobile-версия блока заявки на продуктовых и сервисных страницах уплотнена точечно через `#lead-form`, чтобы не затрагивать контактный блок главной страницы.
...
- `scroll-behavior`;
- `scroll-padding-top`;
- отключение системной голубой tap-подложки на touch-устройствах через `-webkit-tap-highlight-color: transparent`;
- `touch-action: manipulation` для ссылок и кнопок.
...
На этапе mobile UX Optimization выполнены точечные улучшения без изменения backend, API и desktop-логики.

Основные изменения:

- уплотнены карточки продукции на главной;
- карточки услуг приведены к более компактному mobile-формату;
- H1 продуктовых и сервисных страниц сбалансированы по размеру на mobile;
- AI Summary стал компактнее и не конкурирует с основным заголовком;
- ProductDetailsAccordion и FAQ стали плотнее и удобнее на 390 px;
- контактные карточки на главной стали раскрывающимися по логике FAQ;
- блок заявки на продуктовых и сервисных страницах уплотнён через `#lead-form`;
- mobile footer упорядочен и сокращён;
- EntitySwitcher стал легче визуально и не создаёт лишнюю подложку;
- переключатель услуг помещается в одну строку;
- отключена системная голубая tap-подложка на touch-элементах.

Принцип этапа:

```text
улучшаем mobile UX без добавления новых функций,
без изменения backend и без ухудшения desktop-версии
```
...
Mobile footer уплотнён и перестроен в порядке:

```text
бренд и краткое позиционирование
контакты
навигация
экосистема
юридический минимум и политика
```

Ссылка `Политика` из навигации footer удалена, чтобы не дублировать ссылку `Политика обработки персональных данных` в юридическом блоке.
...
- отсутствие системной голубой tap-подложки на touch-элементах;
- единое заполнение рамки изображениями в галерее кейса.
...
- `Проект`, `СМР` и `ПНР` помещаются в один segmented control на mobile;
...
- выполнена mobile UX-оптимизация продуктовых страниц;
- выполнена mobile UX-оптимизация сервисных страниц;
- переключатель услуг приведён к формату `Проект / СМР / ПНР`;
- добавлен режим `document` для технических изображений и чертежей;
- уплотнены mobile FAQ и ProductDetailsAccordion;
- уплотнён блок заявки на продуктовых и сервисных страницах через `#lead-form`;
- обновлён mobile Footer: сокращён текст, удалён дубль `Политика`, удалён `Теплоучет` из экосистемы;
- исправлен URL кейса БТП Дербент;
- галерея CaseModal переведена на единое заполнение рамки изображениями;
- отключена системная голубая tap-подложка на mobile touch-элементах;
...
1. Провести общий контрольный mobile-проход: 360, 390, 430, 768 и 960 px.
2. Проверить все продуктовые страницы после mobile UX-оптимизации.
3. Проверить все сервисные страницы после выравнивания с продуктовой логикой.
4. Завершить и проверить контент всех кейсов.
5. Проверить alt-тексты и подписи всех галерей.
6. Оптимизировать изображения кейсов.
7. Обновить и проверить `sitemap.xml` с маршрутами `/cases/:slug`, если прямые страницы кейсов должны индексироваться.
8. Выполнить production-deploy последних frontend-изменений.
9. Провести production smoke-test модальных и прямых кейсов.
10. Повторить Lighthouse для главной, `/cases` и прямых кейсов.
11. Продолжить работу с Core Web Vitals.
12. Постепенно разделить `App.css` по компонентам.
13. Подключить аналитику после согласования cookies.
14. Передать юридический текст на финальную проверку юристу.
15. Расширять продуктовый и сервисный контент.
16. Подготовить будущие CRM-интеграции.
17. Отдельно рассмотреть page transition animation без риска для scroll-сценариев.
